...
import { Sparkles, Send, Mic, MicOff, X, Volume2, VolumeX, User, Calendar, Power, Thermometer, ChevronDown, Check, RefreshCw } from 'lucide-react';
...
export default function AiChatbot({ selectedDevice, token, sendControl }) {
  ...
  const toggleRecording = () => {
    ...
    if (isRecording) {
      ...
      try {
        ...
        // Sometimes instance is in a bad state, recreate and retry
        try {
          recognitionRef.current?.abort();
          recognitionRef.current?.start();
        } catch (err) {
  ...
  return (
    <>
      {/* Waveform & Fullscreen Keyframe Animations Injection */}
      <style>{`
