...
export default function Analytics({ deviceId, token }) {
  ...
  const [timeframe, setTimeframe] = useState('7d'); // '24h', '7d', '12m'
  const [metric, setMetric] = useState('hours'); // 'hours', 'kWh'
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      ...
      try {
        const response = await fetch(`${API_BASE}/api/analytics?deviceId=${deviceId}&days=${queryDays}`, {
          headers: {
        ...
        // Transform the raw events into graphable points with all states preserved
        const chartData = result.sessions.map(s => ({
  ...
  const getTimeframeTotals = () => {
    ...
    const segments = buildAllHistorySegments(data);
    const buckets = timeframe === '24h' 
      ? generate24HourBuckets() 
      : timeframe === '7d' 
      ? generate7DayBuckets() 
      : generate12MonthBuckets();
    ...
    segments.forEach(seg => {
      ...
      const durationHours = overlapMs / (1000 * 60 * 60);
      const wattage = seg.wattage || 1000;
      totalKWhVal += (wattage * durationHours) / 1000;
    });

    const hours = totalMs / (1000 * 60 * 60);
    const kWh = totalKWhVal.toFixed(2);
  ...
  // Calculate totals dynamically for the active timeframe
  const { hours: timeframeHoursVal, kWh: timeframeKWh } = getTimeframeTotals();
  ...
  return (
    <div className="space-y-6 pb-12">
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <YAxis 
                ...
                dx={-4}
                label={{ 
                  value: metric === 'hours' ? 'Active Time' : 'Energy Consumed (kWh)', 
                  angle: -90, 
                  position: 'insideLeft', 
          ...
          <div className="flex flex-wrap gap-2 w-full lg:w-auto">
            {[
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full text-[9px] font-bold tracking-wide border transition-all ${
                ...
                <div className="relative pl-4 border-l border-slate-800 ml-2.5 space-y-5">
                  {groupedEvents[dateGroup].map(event => (
                    <div key={event.id} className="relative group">
                      {/* Left Dot */}
                      <span
                        className={`absolute -left-[21px] top-1.5 w-2 h-2 rounded-full border border-slate-950 ring-4 ring-slate-950 z-10 ${
                      ...
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-1">
                        ...
                        {/* Badge */}
                        <span
                          className={`text-[8px] font-bold uppercase tracking-widest px-2 py-0.5 rounded border self-start sm:self-auto ${
                            event.category === 'alerts'
      ...
      {/* 3. Live Power Consumption Area Chart */}
      <div className="glass-card p-4">
        <div className="flex items-center gap-2 mb-4 px-2">
          <Zap className="w-4 h-4 text-amber-400" />
          <h3 className="text-xs font-semibold text-slate-300 uppercase tracking-wider">Detailed Wattage History</h3>
        </div>
        <div className="h-40 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data}>
              ...
              />
              <Area type="monotone" dataKey="wattage" stroke="#fbbf24" strokeWidth={1.5} fillOpacity={1} fill="url(#colorWattage)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
